...
const Chatbot = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/api/recommendations/chatbot/",
        {
          method: "POST",
          headers: {
  ...
  return (
    <>
      <Navbar />
      <div
        className="relative min-h-screen flex flex-col items-center justify-center p-6 bg-gradient-to-br from-gray-900 to-black"
        style={{
          ...
          <div className="w-full p-6 sm:p-8 rounded-3xl border border-blue-500/30 bg-gradient-to-b from-gray-900/90 to-black/90  shadow-[0_0_15px_rgba(0,255,255,0.1)] transition-all duration-300 hover:shadow-[0_0_25px_rgba(0,255,255,0.2)] hover:border-blue-400/40 ">
                      ...
                      {parts.map((part, partIndex) => {
                        ...
                        if (isDoubleAsterisk) {
                          return (
                            <h2
                              key={partIndex}
                              className="text-lg font-bold mb-2 border-b border-white/20 pb-1 animate-slideIn"
                            >
                              {part.replace(/\*\*/g, "")}
                            </h2>
                          );
                          ...
                          return (
                            <h3
                              key={partIndex}
                              className="text-base font-extrabold mb-1 animate-slideIn"
                            >
                              {part.replace(/\*/g, "")}
                            </h3>
                          );
                          ...
                          return (
                            <span
                              key={partIndex}
                              className="inline-block mb-1 animate-fadeIn"
                            >
                              {part}
                            </span>
                          );
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              ...
              <button
                className={`${
                  micClick === true
                    ? "bg-green-500 rounded-lg px-2 py-2"
